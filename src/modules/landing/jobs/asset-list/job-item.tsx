...
const JobItem = ({ item, handleDeleteModal }: any) => {
  const navigate = useNavigate();

  return (
    <div
      className="form-shadow rounded-[30px] p-6 dark:bg-[#15171E] relative"
    ...
    >
      ...
      <div className="mt-7">
        ...
        <div className="mt-3">
          <Button
            size={14}
            onClick={() => navigate(`/creator/job/view/${item.id}`)}
            title="View Job"
      ...
      <RiDeleteBin5Line className=" cursor-pointer" size={20} color="red" onClick={handleDeleteModal}/>
      <MdOpenInNew className=" cursor-pointer"  size={20} />
      </div>
    </div>
  );
};
