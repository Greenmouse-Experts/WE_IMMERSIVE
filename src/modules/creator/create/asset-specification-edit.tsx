...
import { editDigitalAsset } from "../../../api/creator";
...
const AssetSpecificationEdit = ({
  ...
  const {
    ...
    mode: "onChange",
    defaultValues: {
      specificationSubjectMatter: assetDetails?.specificationSubjectMatter,
    ...
    const newPayload = {
      ...
      specificationTags:
        typeof specificationTags === "string"
          ? specificationTags.split(",").map((tag) => tag.trim())
          : Array.isArray(specificationTags)
          ? specificationTags.map((tag) => tag.trim())
          : [],
    };
  ...
  return (
    <div>
      <p className="fw-600 text-sm text-grey">EDIT ASSET</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
        ...
        <div className=" mt-16 flex w-full justify-between ">
          <Button
            ...
            size={14}
            title={
              isPending ? <BeatLoader size={12} color="white" /> : "Submit"
            }
            disabled={!isValid}
...
export default AssetSpecificationEdit;
