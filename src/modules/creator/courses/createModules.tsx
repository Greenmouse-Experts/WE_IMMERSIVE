...
import { createCourseModule, deleteCourseModule, getCourseModules, getModulesLesson, getSingleCourse } from "../../../api";
...
const CreateModules = () => {
    ...
    const courses = useGetData(["singleCourse", courseId ? JSON.parse(courseId) : ""], () => getSingleCourse(courseId ? JSON.parse(courseId) : ""));

    const module = useGetData(["courseModules", courseId ? JSON.parse(courseId) : ""], () => getCourseModules(courseId ? JSON.parse(courseId) : ""));
    ...
    const onSubmit = (formData: any) => {
        ...
        if (courseId) {
            ...
            mutation.mutate(payload, {
                onSuccess: () => {
                    if (courseId) {
                        queryClient.invalidateQueries({ queryKey: ["singleCourse", JSON.parse(courseId)] });
                    }
                    if (courseId) {
                        queryClient.invalidateQueries({ queryKey: ["courseModules", JSON.parse(courseId)] });
                    }
                },
            });
        }
    ...
    useEffect(() => {
        if (module.data) {
            const sortedModules = module.data.data.sort((a: { sortOrder: number; }, b: { sortOrder: number; }) => a.sortOrder - b.sortOrder);
    ...
    return (
        <div className="rounded-[20px] py-5 px-7 md:w-3/4 w-full bg-white dark:bg-black">
            <p className="fw-600 text-sm text-grey uppercase">CREATE MODULES ON {course?.title}</p>
            ...
            {modules.length > 0 && modules.map((module, index) => (
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-indigo-50 rounded-t-xl">
                        ...
                        {/* Module Title */}
                        <h3 className="flex-1 text-gray-800 font-medium">{module.title}</h3>
                            ...
                            <Menu placement="left">
                                ...
                                <MenuList>
                                    <MenuItem className="flex flex-col gap-3">
                                        <span className="cursor-pointer w-full" onClick={() => handleModuleDelete(module.id)}>
                                            Delete Module
                                        </span>
                                    </MenuItem>
                                    <MenuItem className="flex flex-col gap-3">
                                        <span className="cursor-pointer w-full" onClick={() => handleDisplayLessons(module.id)}>
                                            View Lessons
                                        </span>
                                    </MenuItem>
                                </MenuList>
                        ...
                        {lessonsArr.length > 0 && selectedId === module.id ?
                            <div className="flex flex-col gap-3 p-2 mb-3">
                                {lessonsArr.map((lesson, index) => (
                                    <p key={index} className="text-sm underline">{lesson.title}</p>
                                ))}
                            </div>
                            :
                            <></>
            ...
            <div className="w-full">
                <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
                    ...
                    <div className="mt-1 flex">
                        <Button
                            style={{ width: "fit-content" }}
                            title={isBusy ? <BeatLoader size={12} color="white" /> : "Create Module"}
                            withArrows
                            disabled={!isValid}
                            size={14}
                            altClassName="btn-primary px-10 py-2 whitespace-nowrap"
            ...
            <Dialog open={open} handler={handleOpen}>
                <DialogBody>
                    <p className="unbound w-full text-center p-2">Are you sure you want to delete this module?</p>
                </DialogBody>
                <DialogFooter>
                    <div className="flex gap-4">
                        ...
                        <Button
                            style={{ width: "fit-content" }}
                            title={isBusy ? <BeatLoader size={12} color="white" /> : "Delete Module"}
                            withArrows
                            size={14}
