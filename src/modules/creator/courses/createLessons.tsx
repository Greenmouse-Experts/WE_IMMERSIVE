    ...
    const onSubmit = (formData: any) => {
        if (id) {
            const payload = {
                ...formData,
                moduleId: id,
                contentUrl: files === '' ? `https://we-imersive.netlify.app${window.location.pathname}` : files
            }
        ...
        return nodes
            .map(node => {
                ...
                if (node.type === 'bulleted-list') {
                    return node.children.map((listItem: { children: any[]; }) => `- ${extractFormattedText(listItem.children)}`).join("\n");
                }
                if (node.type === 'numbered-list') {
                    return node.children.map((listItem: { children: any[]; }, index: number) => `${index + 1}. ${extractFormattedText(listItem.children)}`).join("\n");
                }
                        ...
                        {selectedContentType === 'text' &&
                            <div>
                                ...
                                <div className="w-full p-7 border border-dashed">
                                    <RichTextEditor onChange={(value) => handleEditorChange(value)} />
                                </div>
                            </div>
                        }
